...
export function EnhancedUsersList({ onUserSelect, onUserEdit, onUserDelete }: EnhancedUsersListProps) {
    ...
    useEffect(() => {
        fetchUsers()
    }, [currentPage, searchTerm, activityFilter, sortBy, sortOrder])
    ...
    const handleActivityFilter = (status: string) => {
    ...
    const handleCleanupDuplicates = async () => {
        try {
            ...
            const response = await fetch('/api/users/enhanced?action=cleanup_duplicates', {
                method: 'DELETE'
            })
            const data = await response.json()
    ...
    const getActivityStatusIcon = (status: string) => {
        switch (status) {
            ...
            case 'dormant': return <AlertTriangle className="w-4 h-4" />
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {users.map((user) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getActivityStatusColor(user.activity_status)}`}>
                                            <div className="flex items-center space-x-1">
                                                {getActivityStatusIcon(user.activity_status)}
                                                <span>{getActivityStatusText(user.activity_status)}</span>
                                            </div>
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
