-- Полная система callback с красивым UI/UX
...
DROP FUNCTION IF EXISTS update_callback_conversation_count();
DROP FUNCTION IF EXISTS crm_users_upsert(text, text, text, uuid);

-- 2. Создаем таблицу для переписки по обращениям
CREATE TABLE IF NOT EXISTS callback_conversations (
    ...
    is_internal BOOLEAN DEFAULT false, -- для внутренних заметок админа
    file_url TEXT NULL,
    file_name TEXT NULL,
...
-- 4. Обновляем таблицу callback_requests - добавляем поля для UI
...
-- 5. Создаем функцию для безопасного upsert в crm_users (без user_id)
...
BEGIN
    ...
    -- Upsert в crm_users (без user_id, так как его нет в таблице)
...
-- 6. Функция для обработки callback_requests (БЕЗ создания пользователей)
...
-- 7. Создаем триггер для обработки пользователей
...
-- 8. Функция для создания уведомлений и обновления счетчиков
...
-- 9. Создаем триггер для уведомлений
...
-- 10. Функция для обновления счетчиков переписки
...
-- 11. Создаем триггер для обновления счетчиков
...
-- 12. Создаем представление для удобного просмотра обращений с перепиской
...
-- 13. Создаем RLS политики для безопасности
...
-- Пользователи могут создавать сообщения только в своих обращениях
...
-- 14. Создаем функцию для получения обращений пользователя
...
-- 15. Создаем функцию для получения переписки по обращению
