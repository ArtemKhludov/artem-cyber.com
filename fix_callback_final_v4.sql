-- Финальная версия триггера с правильными колонками issue_reports
...
-- Функция для создания обращения из заявки с правильными колонками
...
BEGIN
    ...
    IF NEW.user_id IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'issue_reports') THEN
        -- Создаем issue с правильными колонками
