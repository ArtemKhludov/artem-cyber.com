...
class ContraAdapter(BaseSiteAdapter):
...
def load_default() -> ContraAdapter:
    root = Path(__file__).resolve().parents[3] / "config" / "sites" / "contra.yaml"
    return ContraAdapter(root)
