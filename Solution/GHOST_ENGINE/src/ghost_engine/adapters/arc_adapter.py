...
class ArcDevAdapter(BaseSiteAdapter):
...
def load_default() -> ArcDevAdapter:
    root = Path(__file__).resolve().parents[3] / "config" / "sites" / "arc_dev.yaml"
    return ArcDevAdapter(root)
