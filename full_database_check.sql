-- ПОЛНАЯ ПРОВЕРКА БАЗЫ ДАННЫХ И СОЗДАНИЕ ВСЕГО НЕОБХОДИМОГО
...
AND tablename IN ('users', 'callback_requests', 'callback_conversations', 'crm_users', 'callback_notifications')
ORDER BY tablename;

-- Проверяем структуру callback_requests
SELECT 
    column_name,
    data_type,
    is_nullable,
...
-- Проверяем существующие ограничения в crm_users
SELECT 
...
-- Проверяем дубликаты в crm_users
...
FROM pg_proc 
WHERE proname IN ('create_user_from_callback', 'create_issue_from_callback', 'update_callback_conversation_count', 'crm_users_upsert', 'get_user_callbacks', 'get_callback_conversation');
...
-- Удаляем дубликаты из crm_users (оставляем только первую запись)
...
-- ========================================
-- 3. СОЗДАНИЕ НЕДОСТАЮЩИХ ТАБЛИЦ И ПОЛЕЙ
-- ========================================
...
-- Создаем уникальное ограничение на email в crm_users (если не существует)
DO $$
DECLARE
...
-- Проверяем все созданные компоненты
SELECT 'ТАБЛИЦЫ:' as component_type, count(*) as count
FROM information_schema.tables 
WHERE table_name IN ('callback_conversations', 'callback_requests', 'crm_users', 'users', 'callback_notifications')
UNION ALL
...
WHERE table_name = 'callback_requests_with_conversations';

-- Тестируем функцию crm_users_upsert
SELECT crm_users_upsert('test-final@example.com', 'Финальный Тест', '1234567890') as test_result;
